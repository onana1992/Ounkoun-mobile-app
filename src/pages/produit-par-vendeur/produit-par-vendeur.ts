...
import { FiltreModalVendeurPage} from '../filtre-modal-vendeur/filtre-modal-vendeur';
...
export class ProduitParVendeurPage {
    ...
    constructor(public navCtrl: NavController, public popoverCtrl: PopoverController, public navParams: NavParams,private restProvider: RestProvider,
		...
		this.shareUrl="https://www.ounkoun.com/boutique.php?nom="+this.sellerName;
		...
		this.restProvider.getSeller(this.sellerName).subscribe(
	        data => {
	            this.idImage = data.data.idImage;
	            console.log(data);
		...
		this.restProvider.getProduitPerSeller(this.sellerName,this.page,this.filterOption,this.filterMinPrice,this.filterMaxPrice,this.filterMarque).subscribe(
	        data => {
  ...
  addFavorite(produit:any, event: Event){
    ...
    presentActionSheet(produitName,modelName) {
  	...
  	showFiltreModal(){
		let myModal = this.modalCtrl.create(FiltreModalVendeurPage, { vendeur: this.sellerName  });
		myModal.present();
